...
const SalesPage = () => {
  ...
  const saleTotal = isFree ? 0 : (selectedProduct ? (selectedProduct.salePrice || 0) * qty : 0);
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 18, overflow: 'hidden', boxShadow: 'var(--shadow-card)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', maxHeight: 400 }}>
            {loadingProducts ? (
              ...
              return (
                <button key={p.id} onClick={() => { if (!isOut) { setSelectedProduct(p); setQty(1); } }} disabled={isOut}
                  style={{
